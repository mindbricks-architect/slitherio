...
const { dbScriptGetPlayersession } = require("dbLayer");

class GetPlayerSessionManager extends PlayerSessionManager {
  ...
  async doBusiness() {
    const playersession = await dbScriptGetPlayersession(this);
  ...
  async raiseEvent() {
    PlayersessionRetrivedPublisher.Publish(this.output, this.session).catch(
      (err) => {
...
module.exports = GetPlayerSessionManager;
