...
class CreatePlayerSessionMcpController extends PlayerSessionMcpController {
  constructor(params) {
    super("createPlayerSession", "createplayersession", params);
    this.dataName = "playerSession";
    this.crudType = "create";
  }

  createApiManager() {
    return new CreatePlayerSessionManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "Instantly creates a temporary session for a guest player. Assigns unique sessionToken, sets connectionStatus, and joins the global environment.",
    parameters: CreatePlayerSessionMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new CreatePlayerSessionMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return CreatePlayerSessionMcpController.getOutputSchema().parse(result);
