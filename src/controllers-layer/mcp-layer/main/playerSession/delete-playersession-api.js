...
class DeletePlayerSessionMcpController extends PlayerSessionMcpController {
  constructor(params) {
    super("deletePlayerSession", "deleteplayersession", params);
    this.dataName = "playerSession";
    this.crudType = "delete";
  }

  createApiManager() {
    return new DeletePlayerSessionManager(this.request, "mcp");
  }
...
module.exports = (headers) => {
  return {
    ...
    description:
      "End a player session: remove from active list, trigger avatar removal, and broadcast final summary for game exit.",
    parameters: DeletePlayerSessionMcpController.getInputScheme(),
    controller: async (mcpParams) => {
      console.log("Mcp Request Received", mcpParams);
      mcpParams.headers = headers;
      const controller = new DeletePlayerSessionMcpController(mcpParams);
      try {
        const result = await controller.processRequest();
        //return DeletePlayerSessionMcpController.getOutputSchema().parse(result);
