...
// This script is called from the DeletePlayerSessionManager
...
const dbScriptDeletePlayersession = async (apiManager) => {
  const whereClause = apiManager.whereClause;

  try {
    const res = await PlayerSession.destroy({ where: whereClause });
    const dbData = res ? { id: apiManager.playerSessionId } : null;

    if (!dbData) {
      throw new NotFoundError("errMsg_RecordNotFoundToDelete");
    }

    apiManagerplayerSession = dbData;
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptDeletePlayersession",
      {
...
module.exports = dbScriptDeletePlayersession;
