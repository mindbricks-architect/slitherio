...
// This script is called from the CreatePlayerSessionManager

const { PlayerSession } = require("models");

const { PlayerSessionQueryCacheInvalidator } = require("./query-cache-classes");
...
async function updateIfExists(dataClause) {
  const dbDoc = await PlayerSession.findByPk(dataClause.id);
  if (dbDoc) {
...
async function checkForUniqueIndex(dataClause) {
  const whereClause = {
    sessionToken: dataClause.sessionToken,
  };
  const dbDoc = await PlayerSession.findOne({ where: whereClause });
  if (dbDoc) {
    indexName = "uniqueSessionToken";
...
const dbScriptCreatePlayersession = async (apiManager) => {
  ...
  try {
    ...
    if (!updated) {
      const dbDoc = await PlayerSession.create(dataClause);
      dbData = dbDoc ? dbDoc.getData() : null;
    }

    apiManager.playerSession = dbData;
    ...
    const queryCacheInvalidator = new PlayerSessionQueryCacheInvalidator();
    ...
    throw new HttpServerError(
      "errMsg_dbErrorWhenExecuting_dbScriptCreatePlayersession",
      {
        whereClause: null,
...
module.exports = dbScriptCreatePlayersession;
